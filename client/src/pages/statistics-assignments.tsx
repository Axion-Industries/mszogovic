...
export default function StatisticsAssignments() {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center space-x-3">
              ...
              <h1 className="text-xl font-semibold text-edu-text dark:text-white">AP Statistics Assignments</h1>
            </div>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {assignments && (
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
            <Card className="bg-edu-green/10 dark:bg-edu-green/20 border-edu-green animate-in slide-in-from-left duration-500">
              <CardContent className="p-4 text-center">
                <Target className="w-8 h-8 text-edu-green mx-auto mb-2" />
                <p className="text-2xl font-bold text-edu-green">{assignments.length}</p>
                <p className="text-sm text-gray-600 dark:text-gray-300">Total Assignments</p>
              </CardContent>
            </Card>
            <Card className="bg-edu-blue/10 dark:bg-edu-blue/20 border-edu-blue animate-in slide-in-from-bottom duration-500">
              <CardContent className="p-4 text-center">
                <ListTodo className="w-8 h-8 text-edu-blue mx-auto mb-2" />
                <p className="text-2xl font-bold text-edu-blue">{assignments.filter(a => a.status === 'active').length}</p>
            ...
            <Card className="bg-edu-accent/10 dark:bg-edu-accent/20 border-edu-accent animate-in slide-in-from-right duration-500">
              <CardContent className="p-4 text-center">
                ...
                <p className="text-2xl font-bold text-edu-accent">{assignments.filter(a => a.category === 'project').length}</p>
                    ...
                    <div className="flex flex-col sm:items-end space-y-2">
                      <Badge 
                        ...
                        className={`text-xs ${
                          ...
                          assignment.category === 'quiz' ? 'bg-edu-green text-white border-edu-green' :
                          assignment.category === 'project' ? 'bg-edu-blue text-white border-edu-blue' :
